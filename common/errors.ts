...
export function createJFrogError(status: number, response: { message?: string; errors?: Array<{ message: string }>; reset_at?: string }): JFrogError {
  switch (status) {
  case 401:
  ...
  case 500:
    return new JFrogResourceNotFoundError(response?.message || "Resource");
  default: {
    const errorMessage = response?.errors?.[0]?.message || response?.message || "JFrog API error";
